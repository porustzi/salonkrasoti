    ...
    <>
      ...
      <section className="relative min-h-[85vh] md:min-h-screen flex items-center overflow-hidden bg-neutral-900">
        <div className="absolute inset-0 z-0">
          <img src={c.hero.backgroundImage} alt="" aria-hidden="true" className="w-full h-full object-cover opacity-50" />
          ...
          <div className="max-w-2xl">
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
              ...
              >
                ...
                <span className="text-champagne text-sm font-medium tracking-[0.25em] uppercase">{c.hero.eyebrow}</span>
              </motion.div>
              <motion.h1
              ...
              <motion.div
                initial={{ opacity: 0, scaleX: 0 }} animate={{ opacity: 1, scaleX: 1 }} transition={{ duration: 0.6, delay: 0.5 }}
              ...
              <motion.div
                initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.8 }}
                className="flex flex-wrap gap-4"
              >
                <Button onClick={openBooking} showArrow>{c.hero.ctaText}</Button>
                <Button to={c.hero.secondaryCtaLink} variant="secondary" className="!border-white/40 !text-white hover:!bg-white hover:!text-neutral-900">{c.hero.secondaryCtaText}</Button>
              </motion.div>
              <motion.div
                initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 1 }}
          ...
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
            ...
            >
              ...
              <div className="absolute -bottom-6 -right-6 bg-champagne rounded-2xl p-6 shadow-lg hidden sm:block">
                ...
                <p className="text-sm text-neutral-700">{c.aboutPreview.statLabel}</p>
              </div>
            </motion.div>
            <motion.div
              initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}
            >
              <span className="text-champagne text-sm font-medium tracking-widest uppercase">{c.aboutPreview.label}</span>
              <h2 className="text-3xl md:text-4xl font-heading font-bold text-neutral-900 mt-4 mb-6">{c.aboutPreview.heading}</h2>
              <p className="text-neutral-600 mb-6">{c.aboutPreview.text}</p>
              <ul className="space-y-3 mb-8">
                {c.aboutPreview.features.map((item, index) => (
              ...
              <Button to={c.aboutPreview.ctaLink} variant="secondary" showArrow>{c.aboutPreview.ctaText}</Button>
            </motion.div>
          </div>
        </div>
      </section>
      ...
      <section className="section-padding bg-cream">
        <div className="container-custom">
              ...
              >
                ...
                >
                  ...
                  <div className="p-5">
                    ...
                    <span className="text-sm font-medium text-champagne flex items-center gap-1">Детальніше <ArrowRight className="w-4 h-4" /></span>
                  </div>
                </Link>
              </motion.div>
            ))}
          ...
          >
            <Button to="/pricing" showArrow>Всі послуги</Button>
          </motion.div>
        </div>
      </section>
      ...
      <section className="section-padding bg-white">
        <div className="container-custom">
          ...
          <motion.div
            ...
            className="text-center mt-12"
          >
            <Button to="/gallery" variant="secondary" showArrow>Дивитися галерею</Button>
          </motion.div>
        </div>
      </section>
      ...
      <section className="section-padding bg-beige">
        <div className="container-custom">
            ...
            {previewReviews.map((review, index) => (
              ...
              >
                <div className="flex items-center gap-1 mb-4">
                  {[...Array(5)].map((_, i) => (
                    <Star key={i} className={`w-4 h-4 ${i < review.rating ? 'text-champagne fill-champagne' : 'text-neutral-200'}`} />
                  ))}
                </div>
                <p className="text-neutral-700 mb-4 line-clamp-4">{review.text}</p>
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-champagne/20 flex items-center justify-center text-champagne font-medium">{review.author.charAt(0)}</div>
                  <div>
          ...
          <motion.div
            ...
            className="text-center mt-12"
          >
            <Button to="/reviews" showArrow>Всі відгуки</Button>
          </motion.div>
        </div>
      </section>
      ...
      <section className="section-padding bg-neutral-900 text-white">
        <div className="container-custom">
          ...
          >
            <h2 className="text-3xl md:text-4xl font-heading font-bold mb-6">{c.cta.heading}</h2>
            <p className="text-neutral-400 mb-8">{c.cta.text}</p>
            <div className="flex flex-wrap justify-center gap-4">
              <Button onClick={openBooking} variant="gold" showArrow>{c.cta.ctaText}</Button>
              <Button to={c.cta.secondaryCtaLink} variant="ghost">{c.cta.secondaryCtaText}</Button>
            </div>
          </motion.div>
        </div>
      </section>
